...
> **Integración UC-308:** el coordinador de shutdown UC-324 puede inyectarse como `ShutdownAdapter` para stop, cancelación y reconciliación. Es un contrato explícito; UC-308 no descubre ni auto-conecta contención productiva.
...
![alt text](../UC-317/uc315_uc317_uc324_sequence_diagram.png)
...
.venv/bin/python3 UC-324.py --demo-safe-shutdown
.venv/bin/python3 UC-324.py --demo-safe-shutdown-contained
...
| POST | `/api/v1/containment/safe-shutdown` | Safe shutdown determinista con coordinación de subsistemas |
| GET | `/api/v1/containment/shutdown-status` | Estado actual del SafeShutdownCoordinator |
| GET | `/api/v1/containment/shutdown-postmortem` | Postmortem redactado del último shutdown |
| GET | `/api/v1/containment/shutdown-evidence` | Cadena de evidencia inmutable del shutdown |
| POST | `/api/v1/containment/reactivation-request` | Solicitar reactivación humana tras shutdown |
...
![alt text](../UC-317/uc315_uc317_uc324_iteration.png) 
...
python3 -m pytest tests/test_safe_shutdown.py -q --tb=short
...
└── code/
    ...
    ├── safe_shutdown_models.py      # L: State machine, evidence chain, models
    ├── safe_shutdown_coordinator.py  # L: SafeShutdownCoordinator + adapter protocols
    ├── policies/
    ...
    └── tests/
        ├── __init__.py
        ├── test_containment.py
        └── test_safe_shutdown.py     # 44 tests for SafeShutdownCoordinator
```
...
## 9.1. SafeShutdownCoordinator (L) — Safe Deterministic Shutdown

UC-324 extends its containment protocol with a **SafeShutdownCoordinator**
that orchestrates a deterministic, idempotent, auditable safe shutdown of
the autonomous agent system, coordinating with multiple subsystems.
...
- **QUIESCING**: Rejecting new tasks; quiescing tool gateway (UC-300).
- **DRAINING**: Draining/cancelling active tasks (UC-317); revoking credentials (UC-300).
- **ROLLING_BACK**: Executing registered simulated transaction rollbacks.
- **CAPTURING**: Building redacted postmortem; memory snapshot (UC-296/UC-326); ingesting into observability (UC-309).
- **SAFE_STOPPED**: Terminal state. Requires human-approved reactivation.
- **CONTAINED**: Terminal failure state with metadata. Requires human-approved reactivation.

### Design Principles

1. **No autonomous restart**: SAFE_STOPPED/CONTAINED require explicit human approval with exact state hash, TTL, anti-replay, and health checks.
2. **Deterministic/offline/simulated**: No real SIGKILL, network calls, DB destruction, or filesystem deletion.
3. **Idempotent**: Same `shutdown_id` returns same status.
4. **Correlated**: Every action is tagged with `shutdown_id` + `trace_id`.
5. **Immutable evidence chain**: Hash-linked entries for forensic integrity.
6. **Adapter protocols**: Each subsystem integrates via a protocol with safe no-op defaults.
7. **Fail-closed**: Adapter failures transition to CONTAINED, never crash.

### Adapter Integration

| Adapter | UC | Protocol | Function |
|---|---|---|---|
| `ToolGatewayAdapter` | UC-300 | `quiesce()`, `revoke_pending()`, `status()` | Reject auth, revoke tokens/credentials |
| `SchedulerAdapter` | UC-317 | `stop_accepting_tasks()`, `drain_tasks()`, `status()` | Stop accepting, drain/cancel tasks |
| `ObservabilityAdapter` | UC-309 | `ingest_postmortem()` | Ingest redacted postmortem into immutable store |
| `MemorySnapshotAdapter` | UC-296/326 | `redacted_snapshot()` | Hash/manifest + counts, no raw memory |
| `ReactivationApprovalAdapter` | UC-290 | `request_reactivation()` | Human-reviewed approval with exact hash |

### Reactivation Requirements

1. Current state must be SAFE_STOPPED or CONTAINED.
2. `shutdown_id` must match exactly.
3. `recovery_state_hash` must match the computed hash of the stopped state.
4. Non-empty `reviewer_id` (human reviewer required, cannot auto-approve).
5. `request_id` must not have been used before (anti-replay).
6. Request must not have exceeded its TTL.
7. All registered health checks must pass.
8. UC-290 reactivation approval adapter must approve.
...
coordinator.rollback_registry.register("tx1", lambda: {"rolled_back": True})
coordinator.health_checker.register(lambda: {"healthy": True})

# Initiate shutdown
status = coordinator.initiate_shutdown(reason="manual", shutdown_id="sd-001")
print(status.state)  # SAFE_STOPPED

# Evidence chain
assert coordinator.evidence.verify()

# Reactivation (requires human reviewer)
req = ReactivationRequest(
    shutdown_id="sd-001",
    recovery_state_hash=coordinator.compute_recovery_state_hash(),
...
curl -X POST http://localhost:5299/api/v1/containment/safe-shutdown \
  -H 'Content-Type: application/json' \
  -d '{"reason": "manual", "shutdown_id": "sd-001"}'

# Status
curl http://localhost:5299/api/v1/containment/shutdown-status

# Postmortem
curl http://localhost:5299/api/v1/containment/shutdown-postmortem

# Evidence chain
curl http://localhost:5299/api/v1/containment/shutdown-evidence

# Request reactivation
curl -X POST http://localhost:5299/api/v1/containment/reactivation-request \
  -H 'Content-Type: application/json' \
  -d '{
    "shutdown_id": "sd-001",
    "recovery_state_hash": "<exact_hash>",
    "reviewer_id": "human-1",
    "justification": "Incident resolved"
  }'
```
...
```bash
python3 UC-324.py --demo-safe-shutdown
python3 UC-324.py --demo-safe-shutdown-contained
```

### Tests

```bash
python3 -m pytest tests/test_safe_shutdown.py -v
```

44 tests covering:
- Full happy path (RUNNING -> SAFE_STOPPED)
- Idempotency (same shutdown_id)
- Task rejection after quiesce
- Drain/cancel active tasks
- Rollback (success + failure -> CONTAINED)
- Postmortem integrity (no raw secret leakage)
- Adapter failure -> CONTAINED (each adapter)
- Evidence chain integrity + tampering detection
- Reactivation: hash mismatch, expired TTL, replay, failed health, no reviewer,
  denied by adapter, wrong shutdown_id
- Successful human-approved reactivation
- ContainmentSandbox integration
- Model serialization
...
```text
UC-324/code/
  safe_shutdown_models.py        # State machine, models, evidence chain
  safe_shutdown_coordinator.py   # Main coordinator with adapter protocols
  tests/test_safe_shutdown.py    # 44 comprehensive tests
```

## Integración UC-308 Champion–Challenger

`SafeShutdownCoordinator.stop_experiment()` ofrece un contrato inyectable para detener un experimento, cancelar órdenes paper pendientes, ejecutar el rollback registrado y producir evidencia de reconciliación. El adaptador opera sin órdenes reales y conserva la consistencia de posiciones, efectivo y auditoría simulados.

La integración no se conecta automáticamente en producción: UC-308 debe recibir explícitamente una instancia del coordinador como `shutdown_adapter`. Si no se inyecta, el experimento utiliza un adaptador offline seguro para demostración.
