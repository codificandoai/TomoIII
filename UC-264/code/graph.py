"""Compilación del grafo LangGraph para UC-264 - Model-Based Multi-Agent Planner."""
...
def build_agent(config: Optional[AppConfig] = None) -> CompiledStateGraph:
    ...
    world_model = TravelWorldModel(cfg.model, simulator_env)
...
def run_agent(
    ...
    if final_state.get("final_output") is None:
        nodes = ModelBasedNodes(
            config=cfg,
            world_model=TravelWorldModel(cfg.model, TravelWorldSimulator(cfg.world)),
            simulator=MonteCarloSimulator(
                TravelWorldModel(cfg.model, TravelWorldSimulator(cfg.world)), cfg.model
            ),
            critic=PlanCritic(cfg.model),
            planner=PlanGenerator(TravelWorldSimulator(cfg.world), cfg.model),
            executor=TravelWorldSimulator(cfg.world),
        )
