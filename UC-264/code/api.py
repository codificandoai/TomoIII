"""API REST Flask para UC-264 - Model-Based Multi-Agent Travel Planner."""
...
INPUT_CARDS: List[Dict[str, Any]] = [
    {
        ...
        "description": (
            "Planifica un viaje usando un agente basado en modelo. Genera múltiples "
            "planes candidatos, los simula con Monte Carlo, selecciona el mejor y puede "
            "ejecutarlo si se autoriza."
        ),
        "parameters": [
    ...
    },
    {
        ...
        "description": "Envía observaciones reales para actualizar el world model.",
        "parameters": [
...
OUTPUT_CARDS: List[Dict[str, Any]] = [
    {
        "endpoint": "POST /api/v1/model/plan",
        "description": "Resultado de la planificación basada en modelo: candidatos, simulaciones, selección y ejecución.",
        "fields": [
...
def health() -> tuple:
    return _ok({"service": "uc264-model-based-planner", "status": "ready"})
...
def feedback() -> tuple:
    ...
    world_model = TravelWorldModel(_config.model, TravelWorldSimulator(_config.world))
...
if __name__ == "__main__":
    port = int(os.getenv("UC264_PORT", _config.port))
