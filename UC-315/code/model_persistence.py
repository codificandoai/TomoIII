...
class SavedModels:
    ...
    neural_next_return: Optional[Any] = None
...
class ModelPersistence:
    ...
    def paths(self) -> Dict[str, str]:
        return {
            ...
            "neural_next_return": os.path.join(self.output_dir, "neural_next_return_model.joblib"),
    ...
    def save(
        ...
        if neural_model and neural_model._trained:
            ...
            if len(neural_model._y_next_return) >= 2 and len(set(neural_model._y_next_return)) > 1:
                joblib.dump(neural_model.model_next_return, paths["neural_next_return"])
                saved["neural_next_return"] = paths["neural_next_return"]
    ...
    def load(self) -> SavedModels:
        ...
        if os.path.exists(paths["neural_next_return"]):
            loaded.neural_next_return = joblib.load(paths["neural_next_return"])
    ...
    def attach_to_world_model(
        ...
        if (
            ...
            if loaded.neural_next_return:
                world_model.probabilistic_model.model_next_return = loaded.neural_next_return
        if (
            loaded.gp_model
