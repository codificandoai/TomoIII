"""Simulador de vuelos, hoteles y clima para UC-264 (adaptado de UC-261)."""
...
class TravelWorldSimulator:
    """Simulador determinista del dominio de viajes."""
    ...
    def search_flights(
        ...
        if key not in self._flight_cache:
            ...
            for i, dep in enumerate(departures):
                dep_dt = _parse_dt(f"{date}T{dep}:00")
    ...
    def book_flight(
        self, flight_id: str, details: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        self._sleep()
        ...
        for flights in self._flight_cache.values():
            for f in flights:
                if f["flight_id"] == flight_id:
                    ...
                    # Simular pequeña probabilidad de fallo de reserva
                    if self._rng.random() < 0.05:
                        return {"status": "FAILED", "error": "booking_failed"}
                    f["seats_left"] -= 1
                    return {
                        "status": "BOOKED",
                        "confirmation": f"BK-{flight_id}-{_uuid.uuid4().hex[:6].upper()}",
                        "actual_cost": f["price_usd"],
                    }
    ...
    def book_hotel(
        self, hotel_id: str, details: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        self._sleep()
        import uuid as _uuid
        ...
        for hotels in self._hotel_cache.values():
            for h in hotels:
                if h["hotel_id"] == hotel_id:
                    ...
                    return {
                        "status": "BOOKED",
                        "confirmation": f"HT-{hotel_id}-{_uuid.uuid4().hex[:6].upper()}",
                        "actual_cost": h["price_per_night_usd"],
                    }
