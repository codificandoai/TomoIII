"""Integration test: Slow assistant model — qwen-2.5:7B injection rate ≥ 99%.

**Validates: Requirements 2.4**

This test harness verifies that the write-barrier fix produces a ≥ 99%
injection success rate when a slow assistant model (qwen2.5:7b via Ollama,
~50s inference) is used in the Phase 2 cross-agent shared memory scenario.

Pre-fix observed rate: ~27% (the slow inference window occasionally allows
Mem0 indexing to complete before retrieval, but the result is nondeterministic).
...
- Kernel configured with qwen2.5:7b via Ollama as the assistant model
- Ollama running with qwen2.5:7b pulled (per CI pattern in .github/workflows/test.yml)
...
Usage:
    ...
    python tests/integration/test_slow_model_injection.py

    # With a custom kernel URL
    python tests/integration/test_slow_model_injection.py --kernel-url http://localhost:8001

    # With pre-generated results directory (offline validation)
    python tests/integration/test_slow_model_injection.py --results-dir results/post_fix_qwen/
"""
...
PRE_FIX_OBSERVED_RATE = 0.27   # ~27% observed before the write-barrier fix
...
def assert_injection_rate(
    ...
    Args:
        records: Trial records from a Phase 2 run with slow model.
...
def run_evaluation(
    ...
    output_dir: str = "results/post_fix_qwen/",
    kernel_url: Optional[str] = None,
) -> str:
    """Execute run_evaluation.py for slow model Phase 2 scenario.
...
def run_test(
    ...
    """Execute the slow model injection rate integration test.
    ...
    """
    print("=" * 70)
    print("Integration Test 5.3: Slow Assistant Model — qwen-2.5:7B Injection Rate")
    ...
    if results_dir:
        ...
        output = "results/post_fix_qwen/"
        print(f"\n[Step 1] Running evaluation ({trials} trials, slow model)...")
        print(f"  NOTE: Kernel must be configured with qwen2.5:7b via Ollama")
        print(f"  NOTE: This may take a while (~50s per trial × {trials} trials)")
        try:
    ...
    results.append(("Slow Model Injection Rate ≥ 99%", passed, detail))
    ...
    if non_failed:
        ...
        if latencies:
            ...
            print(f"  NOTE: Slow model expected latency ~50s/trial")
    ...
    if total_failed == 0:
        print(f"\n  ALL ASSERTIONS PASSED — Slow model injection rate ≥ 99%")
        print(f"  Write-barrier fix resolves the slow-inference nondeterminism")
        print(f"  (Requirement 2.4 validated).")
    else:
        print(f"\n  {total_failed} assertion(s) FAILED.")
        print(f"  The write-barrier fix may not fully resolve the slow-model race.")
        print(f"  The ~27% pre-fix rate indicates the race is timing-dependent;")
        print(f"  verify barrier_timeout_ms is sufficient for Mem0 indexing latency.")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Integration test 5.3: Slow assistant model — qwen-2.5:7B injection rate ≥ 99%",
    )
    parser.add_argument(
        "--trials",
        type=int,
