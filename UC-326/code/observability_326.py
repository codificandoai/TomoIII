"""
UC-326 — Observabilidad para MAQRI.

Gestiona métricas, logs estructurados y trazas (spans) para UC-326.
Compatible con Prometheus, Loki y OpenTelemetry.
"""
...
# Cache global de métricas Prometheus para evitar duplicados al crear
# múltiples instancias de ObservabilityManager en el mismo proceso.
_PROMETHEUS_METRICS_CACHE: Dict[str, Any] = {}
...
class ObservabilityManager:
    """Gestiona métricas, logs y spans para UC-326 MAQRI."""
    ...
    def inc_counter(
        ...
        if PROMETHEUS_AVAILABLE:
            if key not in self._prom_counters:
                ...
                metric_key = f"counter:{key}"
                if metric_key not in _PROMETHEUS_METRICS_CACHE:
                    _PROMETHEUS_METRICS_CACHE[metric_key] = Counter(
                        key, f"UC-326 counter: {key}", label_names, registry=REGISTRY
                    )
                self._prom_counters[key] = _PROMETHEUS_METRICS_CACHE[metric_key]
            if labels:
    ...
    def observe_histogram(
        ...
        if PROMETHEUS_AVAILABLE:
            if name not in self._prom_histograms:
                ...
                if metric_key not in _PROMETHEUS_METRICS_CACHE:
                    _PROMETHEUS_METRICS_CACHE[metric_key] = Histogram(
                        name, f"UC-326 histogram: {name}", label_names, registry=REGISTRY
                    )
                self._prom_histograms[name] = _PROMETHEUS_METRICS_CACHE[metric_key]
            if labels:
    ...
    def set_gauge(
        ...
        if PROMETHEUS_AVAILABLE:
            if name not in self._prom_gauges:
                ...
                if metric_key not in _PROMETHEUS_METRICS_CACHE:
                    _PROMETHEUS_METRICS_CACHE[metric_key] = Gauge(
                        name, f"UC-326 gauge: {name}", label_names, registry=REGISTRY
                    )
                self._prom_gauges[name] = _PROMETHEUS_METRICS_CACHE[metric_key]
            if labels:
