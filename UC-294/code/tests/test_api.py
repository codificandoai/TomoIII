...
def test_sam_workspace_endpoint(client):
    from market_data import SyntheticMarketDataGenerator
...
def test_plan_with_sam_endpoint(client):
    from market_data import SyntheticMarketDataGenerator
    ...
    resp = client.post(
        "/api/v1/trading/plan_with_sam",
        data=json.dumps(payload),
        content_type="application/json",
    )
    assert resp.status_code == 200, resp.get_data(as_text=True)
    data = resp.get_json()["data"]
    assert "sam_state" in data
    assert data["status"] in ("done", "awaiting_confirmation", "blocked", "aborted_by_sam")
