...
def build_agent(config: Optional[AppConfig] = None) -> StateGraph:
    ...
    planner = StrategyGenerator(world_model, cfg.model)
    ...
    workflow.add_edge("evaluate", "risk_gate")
...
def run_agent(
    ...
    if final_state.get("final_output") is None:
        ...
        central_brain = CentralBrain(cfg)
        nodes = TradingAgentNodes(
            config=cfg,
