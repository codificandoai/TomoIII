...
Te explico cada cambio con lenguaje sencillo, como si fuera el "cerebro" de un agente que planifica un viaje (vuelos, hoteles, actividades).
...
```bash
# Planificar un viaje sin ejecutar reservas (con MCTS + modelos probabilísticos)
python UC-265.py plan \
  ...
  --travelers 1 \
  --budget 2000 \
  --user-id demo-probabilistic-user \
  --airline Delta \
  --direct
...
```bash
# 1. Planificar con modelo probabilístico y MCTS
...
# 3. Feedback real para reentrenar
...
- El entorno parcialmente observable se simula con ruido gaussiano; se puede enriquecer con modelos de eventos reales (retrasos, cancelaciones).
