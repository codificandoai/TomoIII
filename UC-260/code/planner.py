"""Planificación proactiva del itinerario para UC-260 (adaptado de UC-259)."""
...
def _choose_flight(flights: List[Dict[str, Any]], preferences: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    if not flights:
        return None
...
def _choose_hotel(hotels: List[Dict[str, Any]], preferences: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    if not hotels:
        return None
...
def build_itinerary(
    ...
    if not chosen_outbound:
        missing_info.append(
            f"No flights found from {request.origin} to {request.destination} on {request.departure_date}."
        )
    else:
        itinerary.append(_make_plan_item(
            ...
            confidence=0.85,
            notes=[
                f"Direct={chosen_outbound.get('direct')}",
                f"Seat preference: {prefs.get('seat', 'any')}",
                f"Airline: {chosen_outbound.get('airline')}",
            ],
        ))
        assumptions.append(
    ...
    if request.return_date and chosen_outbound:
        ...
        if not chosen_return:
            missing_info.append(
                f"No return flights from {request.destination} to {request.origin} on {request.return_date}."
            )
        else:
            itinerary.append(_make_plan_item(
                step=3,
    ...
    if meeting_time and chosen_outbound:
        ...
        if (meeting_dt - arrival_dt).total_seconds() < buffer_minutes * 60:
            ...
            assumptions.append(
                f"Meeting moved to {meeting_dt.isoformat()} to allow {buffer_minutes}m buffer after arrival."
            )
        itinerary.append(_make_plan_item(
            step=4,
            item_type="meeting",
