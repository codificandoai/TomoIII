"""Nodos BDI del grafo LangGraph para UC-260."""
...
class BDINodes:
    """Contenedor con estado compartido para los nodos del agente BDI."""
    ...
    def _itinerary_copy(self, state: BDIState) -> List[Dict[str, Any]]:
        return copy.deepcopy(state["itinerary"])

    def _desires_copy(self, state: BDIState) -> List[Dict[str, Any]]:
        return copy.deepcopy(state.get("desires", []))

    def _intentions_copy(self, state: BDIState) -> List[Dict[str, Any]]:
    ...
    # --------------------------------------------------------------------------
    def input_validation_node(self, state: BDIState) -> Dict[str, Any]:
        ...
        # Construir itinerario inicial y deseos
        ...
        # Las reservas de vuelos y hoteles son irreversibles: requieren confirmación
    ...
    # --------------------------------------------------------------------------
    # 1b. Reservas iniciales del itinerario
    # --------------------------------------------------------------------------
    def book_itinerary_node(self, state: BDIState) -> Dict[str, Any]:
    ...
    # --------------------------------------------------------------------------
    # 2. Percepción: consultar predictor de retrasos
    # --------------------------------------------------------------------------
    def perceive_node(self, state: BDIState) -> Dict[str, Any]:
        itinerary = self._itinerary_copy(state)
        reflections: List[str] = []
        ...
        if not result["success"]:
            reflections.append("Delay predictor API unavailable; agent will use fallback assumptions and proceed cautiously.")
            beliefs.append(
                Belief(
    ...
    # --------------------------------------------------------------------------
    # 3. Deliberación: marcar deseos amenazados y elegir estrategia
    # --------------------------------------------------------------------------
    def deliberate_node(self, state: BDIState) -> Dict[str, Any]:
        ...
        def effective_delay_prob(flight_item: Dict[str, Any], belief: Dict[str, Any]) -> float:
            ...
            # Menor confiabilidad historica aumenta la probabilidad efectiva de retraso
        ...
        for flight in self._flight_items(itinerary):
            ...
            if not pred:
                continue
            # Buscar belief asociada a este vuelo
            belief = next(
        ...
        if threatened:
            # Prioridad: llegar a tiempo > evitar retrasos > presupuesto
    ...
    def _airline_reliability(self, airline: str, experiences: List[Dict[str, Any]]) -> float:
        ...
        relevant = [
            ...
            if airline in e.get("context", "")
        ]
        if not relevant:
            return 1.0
        avg = sum(e.get("utility", 0.0) for e in relevant) / len(relevant)
        # Normalizar [-1,1] -> [0.5,1.0]
        return 0.5 + 0.5 * max(-1.0, min(1.0, avg))

    # --------------------------------------------------------------------------
    # 4. Formación de intenciones
    # --------------------------------------------------------------------------
    def intend_node(self, state: BDIState) -> Dict[str, Any]:
    ...
    # --------------------------------------------------------------------------
    # 5. Ejecución de intenciones
    # --------------------------------------------------------------------------
    def execute_node(self, state: BDIState) -> Dict[str, Any]:
        itinerary = self._itinerary_copy(state)
        ...
        for intent in intentions:
            ...
            if action == "finalize" or action == "wait_and_monitor":
                ...
                # Estrategia simple: buscar vuelo más barato para la ruta más cara
    ...
    def _try_reduce_cost(self, itinerary: List[Dict[str, Any]], request: Dict[str, Any]) -> bool:
        # Buscar el vuelo más caro y tratar de encontrar alternativa más barata
    ...
    # --------------------------------------------------------------------------
    # 6. Revisión: evaluar satisfacción de deseos
    # --------------------------------------------------------------------------
    def review_node(self, state: BDIState) -> Dict[str, Any]:
        desires = self._desires_copy(state)
        itinerary = self._itinerary_copy(state)
        reflections: List[str] = []
        ...
        # Llegar a tiempo: si no hay amenaza, marcar satisfecho
        ...
        # Evitar retrasos: satisfecho si no amenazado
        ...
        # Confort: se considera satisfecho si existe un itinerario coherente
        if itinerary:
        ...
        if all_satisfied:
            status = "learning"
            reflections.append("All desires satisfied. Proceeding to learning phase.")
            ...
            status = "learning"
            reflections.append("Max retries reached; finalizing with unresolved desires.")
    ...
    # --------------------------------------------------------------------------
    # 7. Aprendizaje: registrar experiencias y evolucionar
    # --------------------------------------------------------------------------
    def learn_node(self, state: BDIState) -> Dict[str, Any]:
        if not state["request"].get("enable_learning", True):
            return {"status": "finalizing", "reflections": ["Learning disabled."]}
        ...
        for intent in state.get("intentions", []):
            ...
            if status == "COMPLETED" and "Stay within budget" in target:
                # Penalizar levemente si se excedió presupuesto
                budget = state["request"].get("budget")
