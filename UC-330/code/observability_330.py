"""
UC-330 — Observabilidad para Exploitation–Exploration Governance.

Gestiona métricas, logs estructurados y spans para UC-330.
"""
...
class ObservabilityManager:
    """Gestión de observabilidad para UC-330."""
            ...
            if key not in self._prom_counters:
                ...
                if metric_key not in _PROMETHEUS_METRICS_CACHE:
                    _PROMETHEUS_METRICS_CACHE[metric_key] = Counter(
                        key, f"UC-330 counter: {key}", label_names, registry=REGISTRY
                    )
            ...
            if name not in self._prom_histograms:
                ...
                if metric_key not in _PROMETHEUS_METRICS_CACHE:
                    _PROMETHEUS_METRICS_CACHE[metric_key] = Histogram(
                        name, f"UC-330 histogram: {name}", label_names, registry=REGISTRY
                    )
            ...
            if name not in self._prom_gauges:
                ...
                if metric_key not in _PROMETHEUS_METRICS_CACHE:
                    _PROMETHEUS_METRICS_CACHE[metric_key] = Gauge(
                        name, f"UC-330 gauge: {name}", label_names, registry=REGISTRY
                    )
