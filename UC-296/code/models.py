...
# =============================================================================
# Modelos SAM (Situational Awareness Middleware) para UC-294
# =============================================================================
...
class MemoryEpisode(BaseModel):
    """Unidad de memoria episódica de corto plazo."""

    episode_id: str = Field(default_factory=lambda: str(uuid.uuid4())[:8])
    tick: int = 0
    event_type: str = "PERCEPTION"
    content: str = ""
    timestamp: str = Field(default_factory=now_iso)
    metadata: Dict[str, Any] = Field(default_factory=dict)
...
class EnvironmentState(BaseModel):
    """Estado situacional dinámico del entorno ('Aquí y Ahora')."""

    state_id: str = Field(default_factory=lambda: str(uuid.uuid4())[:8])
    system_clock: str = Field(default_factory=now_iso)
...
class SelfModel(BaseModel):
    """Modelo del agente: capacidades, límites, confianza y carga cognitiva."""

    self_model_id: str = Field(default_factory=lambda: str(uuid.uuid4())[:8])
    agent_identity: str = "UC294.Alpha"
    current_goal: str = "Maximizar retorno ajustado por riesgo"
    competence_profile: Dict[str, float] = Field(default_factory=lambda: {
...
class WorkspaceContent(BaseModel):
    """Contenido del workspace global de capacidad limitada (GWT)."""
...
class SAMState(BaseModel):
    """Estado completo del Situational Awareness Middleware."""

    sam_state_id: str = Field(default_factory=lambda: str(uuid.uuid4())[:8])
    self_model: Dict[str, Any] = Field(default_factory=dict)
