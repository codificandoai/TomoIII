...
UC-296 `IntelligentMemoryRouter` provides a redacted snapshot method for UC-324's `SafeShutdownCoordinator`:

- **`redacted_snapshot(shutdown_id)`**: Returns a hash/manifest + counts of memory entries (notepad, vector store), NOT raw memory contents, private memory, or chain-of-thought. The manifest hash is computed over entry counts and the shutdown_id for integrity verification.
