"""
UC-329 — Observabilidad para GraphRAG-GoT.

Gestiona métricas, logs estructurados y trazas (spans) para GraphRAG-GoT.
...
class ObservabilityManager:
    """Gestiona métricas, logs y spans para GraphRAG-GoT."""
            ...
            if key not in self._prom_counters:
                ...
                if metric_key not in _PROMETHEUS_METRICS_CACHE:
                    _PROMETHEUS_METRICS_CACHE[metric_key] = Counter(
                        key, f"UC-329 counter: {key}", label_names, registry=REGISTRY
                    )
            ...
            if name not in self._prom_histograms:
                ...
                if metric_key not in _PROMETHEUS_METRICS_CACHE:
                    _PROMETHEUS_METRICS_CACHE[metric_key] = Histogram(
                        name, f"UC-329 histogram: {name}", label_names, registry=REGISTRY
                    )
            ...
            if name not in self._prom_gauges:
                ...
                if metric_key not in _PROMETHEUS_METRICS_CACHE:
                    _PROMETHEUS_METRICS_CACHE[metric_key] = Gauge(
                        name, f"UC-329 gauge: {name}", label_names, registry=REGISTRY
                    )
