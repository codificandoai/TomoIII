...
# ---------------------------------------------------------------------------
# Additional regression tests requested in the review
# ---------------------------------------------------------------------------

def test_exposure_uses_market_price_not_equity():
    """Ensure exposure = |position| * market_price, not equity/market_value."""
    engine = PaperExecutionEngine(
        model_id="t", version="1.0.0",
        initial_cash=1_000_000.0, default_quantity=100.0,
    )
    events = generate_market_events(n=3, seed=5)
    for ev in events:
        pred = ChampionDemoPredictor().predict(ev, "c1", time.time())
        engine.process_event(ev, pred)
    snap = engine.snapshot()
    expected_exposure = abs(snap.position) * engine._last_market_price
    assert abs(snap.exposure - expected_exposure) < 0.01


def test_common_slippage_assumptions_are_deterministic_and_non_negative():
    from cc_execution_308 import _slippage_linear
    import random
    rng = random.Random(42)
    spread = 0.05
    s1 = _slippage_linear(100.0, 1000.0, spread, rng)
    s2 = _slippage_linear(100.0, 1000.0, spread, random.Random(42))
    assert s1 == s2
    assert s1 >= 0


def test_historical_metrics_have_prediction_sample_count():
    config = ExperimentConfig(min_paired_samples=5, paper_samples=5)
    exp = ChampionChallengerExperiment(config=config)
    exp.register_champion("c", "1", ChampionDemoPredictor())
    exp.register_challenger("d", "2", ChallengerDemoPredictor())
    exp.start("historical")
    ...
    for role in ("champion", "challenger"):
        assert metrics[role].sample_count == 5
        assert metrics[role].bid_mae >= 0
        assert metrics[role].ask_mae >= 0
...
def test_duplicate_event_id_rejected():
    ...
    ev = MarketEvent(
        ...
        bid_size=100.0,
        ask_size=100.0,
        reference_bid=100.05,
        reference_ask=100.15,
    )
    r1 = exp.ingest_event(ev)
    assert r1["success"]
    # same event id but different source ts still rejected? We don't reject duplicate IDs currently.
    # The requirement is duplicate event ID; implement in validate? MarketEvent.validate doesn't check ID.
    # We add a simple check in ingest: if event_id already seen, reject.
    ev2 = MarketEvent(
        event_id="DUPE",
        source_ts=1_700_000_001.0,
        receive_ts=1_700_000_001.0,
        symbol="TEST",
        bid=101.0,
        ask=101.1,
        bid_size=100.0,
        ask_size=100.0,
        reference_bid=101.05,
        reference_ask=101.15,
    )
    r2 = exp.ingest_event(ev2)
    assert not r2["success"]


def test_prediction_model_version_binding_is_enforced():
    class WrongChampion(PredictorAdapter):
        model_id = "c"
        version = "99.0.0"
    ...
    config = ExperimentConfig()
    exp = ChampionChallengerExperiment(config=config)
    exp.register_champion("c", "1.0.0", WrongChampion())
    exp.register_challenger("d", "2.0.0", ChallengerDemoPredictor())
    exp.start("historical")
    ev = generate_market_events(n=1, seed=1)[0]
    result = exp.ingest_event(ev)
    assert not result["success"]
    assert "model/version" in result["reason"]


def test_spread_capture_and_implementation_shortfall_sign():
    from cc_execution_308 import PaperExecutionEngine
    engine = PaperExecutionEngine(
        model_id="t", version="1.0.0",
        initial_cash=1_000_000.0, default_quantity=100.0,
    ...
    from cc_models_308 import Prediction
    pred = Prediction(
        model_id="buy", version="1", event_id=ev.event_id,
        correlation_id="c", predicted_bid=151.0, predicted_ask=151.05,
        confidence=None, prediction_ts=1.0, input_hash=ev.event_hash(),
    )
    result = engine.process_event(ev, pred)
    fills = result["fills"]
    if fills:
        # For a buy, implementation shortfall = expected_price - fill_price; if adverse, negative.
        assert fills[0]["slippage"] >= 0
        # Implementation shortfall sign in metrics is captured as negative when paid above expected.


def test_cannot_approve_continue_or_reject_recommendation():
    config = ExperimentConfig(min_paired_samples=2, paper_samples=2)
    exp = ChampionChallengerExperiment(config=config)
    exp.register_champion("c", "1", ChampionDemoPredictor())
    exp.register_challenger("d", "2", ChallengerDemoPredictor())
    ...
    for ev in generate_market_events(n=2, seed=1):
        exp.ingest_event(ev)
    rec = exp.recommend_promotion()
    # Force awaiting state even if recommendation action is not promote.
    exp.state = ExperimentState.AWAITING_APPROVAL.value
    result = exp.approve_promotion(rec.report_hash, "op", "req-99")
    # If action is not promote, approval should still succeed? The API must not bypass recommendation.
    # The requirement is the approval must bind to the recommended action; if action is reject/continue,
    # promotion should not happen. We enforce: only promote action may be approved.
    if rec.recommended_action != PromotionAction.PROMOTE.value:
        assert not result["success"]
        assert "recommendation action" in result["reason"]


def test_ttl_less_or_equal_zero_rejected():
    class YesHITL:
        def approve_experiment_promotion(self, request):
    ...
    rec = exp.recommend_promotion()
    exp.state = ExperimentState.AWAITING_APPROVAL.value
    result = exp.approve_promotion(rec.report_hash, "op", "req-100", ttl_seconds=0.0)
    assert not result["success"]
    assert "ttl" in result["reason"].lower()


def test_uc317_paper_execution_adapter_injection():
    import os, sys
    uc317_path = os.path.join(os.path.dirname(__file__), "..", "..", "..", "UC-317", "code")
    if uc317_path not in sys.path:
        sys.path.insert(0, uc317_path)
    from paper_execution_service_317 import build_default_paper_execution_service
    svc = build_default_paper_execution_service()
    config = ExperimentConfig(
        experiment_id="uc317-cc-001",
        min_paired_samples=2,
        paper_samples=2,
        max_exposure=10_000_000.0,
        max_drawdown=1.0,
    )
    exp = ChampionChallengerExperiment(config=config, paper_execution_adapter=svc)
    exp.register_champion("c", "1", ChampionDemoPredictor())
    exp.register_challenger("d", "2", ChallengerDemoPredictor())
    exp.start("paper")
    for ev in generate_market_events(n=2, seed=1):
        exp.ingest_event(ev)
    assert len(exp.outcomes["champion"]) == 2
    assert exp.outcomes["champion"][-1]["snapshot"] is not None
    # Assert real_order never reached the engine.
    status = svc.status("uc317-cc-001", "c", "1")
