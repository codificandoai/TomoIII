"""
UC-328 — Observabilidad para ORQUESTA-R.

Gestiona métricas, logs estructurados y trazas (spans) para el
orquestador resiliente. Compatible con Prometheus, Loki y OpenTelemetry.
"""
...
# Cache global de métricas Prometheus para evitar duplicados
...
class ObservabilityManager:
    """Gestiona métricas, logs y spans para ORQUESTA-R."""
            ...
            if key not in self._prom_counters:
                ...
                if metric_key not in _PROMETHEUS_METRICS_CACHE:
                    _PROMETHEUS_METRICS_CACHE[metric_key] = Counter(
                        key, f"UC-328 counter: {key}", label_names, registry=REGISTRY
                    )
            ...
            if name not in self._prom_histograms:
                ...
                if metric_key not in _PROMETHEUS_METRICS_CACHE:
                    _PROMETHEUS_METRICS_CACHE[metric_key] = Histogram(
                        name, f"UC-328 histogram: {name}", label_names, registry=REGISTRY
                    )
            ...
            if name not in self._prom_gauges:
                ...
                if metric_key not in _PROMETHEUS_METRICS_CACHE:
                    _PROMETHEUS_METRICS_CACHE[metric_key] = Gauge(
                        name, f"UC-328 gauge: {name}", label_names, registry=REGISTRY
                    )
